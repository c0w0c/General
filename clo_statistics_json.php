 <?php
	...
	include 'include/PDO.php';
 	...
 	switch ($chart_type) { //判斷報表型態
 		//已總數統計
 		case '':
 			$sql = 'SELECT * FROM clo_app where app_date between "'.$star_date.'" and "'.$end_date.'"';
 			$type = "dep";
 			break;
 		//已部門統計
 		case 'dep':
 			$sql = 'SELECT * FROM clo_app where dep = "'.$chart_key.'" and app_date between "'.$star_date.'" and "'.$end_date.'"';
 			$type = "app_rea";
 			break;
 		//已申領原因統計
 		case 'rea':
 			$sql = 'SELECT * FROM clo_app where app_rea = "'.$chart_key.'" and app_date between "'.$star_date.'" and "'.$end_date.'"';
 			$type = "dep";
 			break;
 	}
