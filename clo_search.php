...
$sql = "SELECT * FROM clo_app ORDER BY id DESC";
...
<div data-role="header" style="overflow:hidden;" class="hea">
  <h1>靜電用品</h1>
<a href="index.html#p1" target="_self" data-icon="back">返回</a>
<div data-role="navbar">
  <ul>
  <li><a href="clo_key_search.php?key=%E5%B0%9A%E6%9C%AA%E8%AE%80%E5%8F%96" target="_self">尚未讀取</a></li>
  <li><a href="clo_key_search.php?key=%E6%BA%96%E5%82%99%E4%B8%AD" target="_self">準備中</a></li>
    <li><a href="clo_key_search.php?key=%E5%B7%B2%E7%99%BC%E9%80%81" target="_self">已發送</a></li>
  </ul>
</div><!-- /navbar -->
                <?php
            echo "<div data-role='controlgroup' data-type='horizontal' class='ui-mini ui-btn-right'>";
            if ( $pages > 1 )  // 顯示上一頁
            echo "<a  href='clo_search.php?Pages=".($pages-1).
            ...
            if ($i != $pages) // 顯示頁碼
            echo "<option value='clo_search.php?Pages=".$i."'>".$i."</option>";
            echo "</form></select>";
...
<thead>
  ...
  <th style="border:1px solid #e6e6e6;text-align: center;vertical-align: middle;" data-priority="persist">領用人</br><span style="width:100%;border-top: 1px #000 solid;">申請人</span></th>
  <th style="border:1px solid #e6e6e6;text-align: center;vertical-align: middle;" data-priority="4">部門</th>
  <th style="border:1px solid #e6e6e6;text-align: center;vertical-align: middle;" data-priority="persist">領用</br>原因</th>
    <th style="border:1px solid #e6e6e6;text-align: center;vertical-align: middle;" data-priority="persist">領用</br>物品</th>
    <th style="border:1px solid #e6e6e6;text-align: center;vertical-align: middle;" data-priority="4">備註</th>
...
<tbody>
    ...
    for ($i = $star_record;$i < $num && $i < $total_record; $i++ ) {
        ...
        $S_Status = substr($Status,0,3)."</br>".substr($Status,3,3)."</br>".
                    substr($Status,6,3)."</br>".substr($Status,9,3)."</br>";
    ...
    switch($Status) {
      ...
      echo "<th style='border:1px solid #e6e6e6;text-align: center;vertical-align: middle;'>".$name."</br>(".$work_id.")</br>
                                                             <hr><span style='color:green;'>".$app_name."</span></th>";
      echo "<th style='border:1px solid #e6e6e6;text-align: center;vertical-align: middle;'>".$dep."</th>";
      echo "<th style='border:1px solid #e6e6e6;text-align: center;vertical-align: middle;'>".substr($app_rea,0,6)."</br>".substr($app_rea,6,6)."</th>";
      echo "<th style='border:1px solid #e6e6e6;text-align: center;vertical-align: middle;'>".$app_clo."</th>";
      echo "<th style='border:1px solid #e6e6e6;text-align: center;vertical-align: middle;'>".$remark."</th>";
      echo "<th style='border:1px solid #e6e6e6;text-align: center;vertical-align: middle;color:#".$color."'>".substr($S_Status,0,-5)."</th>";
      echo "</tr>";
      }
      }
...
<form action="clo_key_search.php" method="GET" data-ajax="false">
<table>
  <tr>
