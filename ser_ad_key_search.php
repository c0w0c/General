...
switch($user_pur){
    ...
    case'2':
      $sql = "SELECT * FROM service WHERE ( id LIKE '".$key."' OR ".
             ...
             "location LIKE '%".$key."%' OR ".
             "err_con LIKE '%".$key."%' OR ".
             "ser_sta LIKE '%".$key."%' ) AND ".
             "name = '".$user_name."' ORDER BY id DESC";
      break;
}
...
<?php
            ...
            if ( $pages > 1 )  // 顯示上一頁
            echo "<a href='ser_ad_key_search.php?key=".$key."&Pages=".($pages-1).
            ...
            if ($i != $pages) // 顯示頁碼
            echo "<option value='ser_ad_key_search.php?key=".$key."&Pages=".$i."'>".$i."</option>";
            echo "</form></select>";
